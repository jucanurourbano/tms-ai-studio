...
def assemble_artifact(state: dict[str, Any]) -> tuple[DatabaseArtifact, bool]:
    ...
    # Observaciones = las de CRITIQUE (que ya incorporó las correcciones aplicadas
    # sobre el LLM en los nodos anteriores) + los descartes del propio ensamblado.
    # Las tres familias son NO silenciosas por regla del proyecto.
    ...
    artifact = DatabaseArtifact(
        ...
        metrics=metrics,
        # Resumen de RECONCILE (INV4). ``None`` cuando la fase no corrió: un
        # artefacto anterior al módulo, o un diseño sin inventario contra el que
        # comparar. El contrato lo admite, así que nada se rompe hacia atrás.
        reconciliation=(
