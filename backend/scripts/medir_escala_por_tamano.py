...
async def main() -> None:
    ...
    _cabecera("§2.bis — UN HALLAZGO DEL CAMINO: el documento se envía DOS VECES")
    print("Sale de la tabla de arriba: entre 16,3 KB y 16,9 KB el `EXTRACT` de un")
    print("texto plano duplica su entrada con un 3% más de documento. No es una")
    print("no-linealidad, es una duplicación, y se comprueba sin LLM ni grafo:")
    print()
    ...
    print()
    print("`TextToCIRAdapter` mete TODO el texto plano en un único elemento SECTION.")
    print("Por debajo del umbral el chunker toma el camino `single_shot` y el")
    print("`context` es el título. Por encima toma el camino por títulos, y ahí")
    print("`_context_for(section)` devuelve el breadcrumb MÁS el texto del elemento")
    print("— que en texto plano es el documento entero. El chunk sale con el")
    print("documento en `context` y otra vez en `text`, y `build_user` manda los dos.")
    print()
    print("Consecuencia: por encima de ~16,4 KB el modo *prompt* paga 2x su entrada")
    print("de EXTRACT, seis veces (una por dimensión). Es el recorte más barato que")
    print("ha aparecido: no cambia ni un prompt ni una decisión del agente.")
