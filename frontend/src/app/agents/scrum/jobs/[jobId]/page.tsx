...
import { ScrumProgressView } from "@/components/scrum/progress-view";
import { ScrumResultView } from "@/components/scrum/scrum-result-view";
...
export default function ScrumJobPage() {
  const params = useParams<{ jobId: string }>();
  const jobId = params.jobId;

  const [job, setJob] = useState<ScrumJobDetail | null>(null);
  ...
  if (job.status === "COMPLETED" || job.status === "COMPLETED_WITH_WARNINGS") {
    return <ScrumResultView job={job} />;
  }

  return <ScrumProgressView job={job} onUpdate={setJob} />;
}
